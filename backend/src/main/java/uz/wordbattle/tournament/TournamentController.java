 ...
 * reading its bracket, answering an invite, and — since {@code
 * TournamentService#createByUser} — organizing one of your own among friends
 * instead of waiting for an admin to run one.
 *
 ...
 * below it — accepting or declining an invite, and the organizer-only actions
 * at the bottom — are about one player's own stake in the bracket and stay
 * that way.
 */
@RestController
...
public class TournamentController {
    ...
    public record CreateRequest(String name, int size) {}
    ...
    public TournamentSummaryDto create(@CurrentUser AuthPrincipal principal, @RequestBody CreateRequest request) {
        TournamentEntity created = tournaments.createByUser(principal.userId(), request.name(), request.size());
